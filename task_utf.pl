...
group_nouns(X,CASE,node(группа_существительных, DETERMINATE, NOUN)) --> determinate_group(X, CASE, DETERMINATE), noun(X, CASE, NOUN).
group_nouns(X,CASE,node(группа_существительных, NOUN)) --> noun(X, CASE, NOUN).

determinate_group(X, CASE, node(группа_определителей, DETERMINATE, DETERMINATE_GROUP)) --> determinate(X, CASE, DETERMINATE), determinate_group(X, CASE, DETERMINATE_GROUP).
determinate_group(X, CASE, node(группа_определителей, DETERMINATE)) --> determinate(X, CASE, DETERMINATE).

group_verb(X, node(группа_глаголов, ADVERB, GROUP_VERB)) --> adverb(X, ADVERB), group_verb(X, GROUP_VERB).
group_verb(X, node(группа_глаголов, VERB, GROUP_NOUNS)) --> verb(X, VERB), group_next_nouns(X, GROUP_NOUNS). 
group_verb(X, node(группа_глаголов, VERB)) --> verb(X, VERB).

group_next_nouns(X, node(группа_существительных, THIS_NOUN, NEXT_NOUNS)) --> group_other_nouns(X, THIS_NOUN), group_next_nouns(X, NEXT_NOUNS).
group_next_nouns(X, node(группа_существительных, NOUN)) --> group_other_nouns(X, NOUN).

group_other_nouns(X, node(группа_существительных, EXCUSE, NOUNS)) --> excuse(X, CASE, EXCUSE), group_nouns(X, CASE, NOUNS).
...
determinate(_,'именительный', node(прилагательное,большой)) --> [большой].
...
determinate(_,'родительный', node(прилагательное,большого)) --> [большого].
...
determinate(_,'дательный', node(прилагательное,большому)) --> [большому].
...
determinate(_,'винительный', node(прилагательное,большого)) --> [большого].
